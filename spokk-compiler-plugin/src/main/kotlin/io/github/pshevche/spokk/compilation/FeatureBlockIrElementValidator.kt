...
import io.github.pshevche.spokk.compilation.FeatureBlockIrElementValidator.State.INIT
...
internal class FeatureBlockIrElementValidator() {
    ...
    enum class State {
        ...
        fun failOnInvalidBlock(currentBlock: FeatureBlockIrElement): Nothing {
            ...
            throw CompilationException(
                "Expected to find one of spokk blocks ${displayNames}, but encountered '${currentBlock.block.displayName}'",
                currentBlock.file,
    ...
    fun assertBlockStructureIsComplete() {
        if (currentState != INIT && !currentState.isExpectation()) {
            ...
            throw CompilationException(
                "Expected to find one of spokk blocks ${displayNames}, but reached the end of the feature method",
                lastBlock?.file,
                lastBlock?.element
            )
        }
